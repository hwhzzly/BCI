...
void CCoreModelDoc::AddOriginalData(double* pd, int size)
{
	...
	//ProcessData(m_EEGOriginalData, m_EEGOriginalData);
...
void CCoreModelDoc::RemoveMeanData(double* pd, int size)
{
	...
	else
	{
		...
		{
			pd[i * 2] = pd[i*m_nChannum] - pd[i*m_nChannum + 1];
			pd[i * 2 + 1] = pd[i*m_nChannum + 2] - pd[i*m_nChannum + 3];
		}
		size = size / m_nChannum * 2;
	}
} 

void CCoreModelDoc::RemoveMeanData2(double* pd, int size)
{
	if (m_nChannum == 2)
	{
		if (m_nMeanLValue == 0|| m_nMeanRValue == 0)
		{
			for (int i = 0; i < size; i++)
			{
				if ((i + 1) % 2) 
			...
			m_nMeanLValue = m_nMeanLValue * 2 / size;
			m_nMeanRValue = m_nMeanRValue * 2 / size;
		}
		else
		{
			for (int i = 0; i < size; i++)
...
void   CCoreModelDoc::Highpass(double* pd,double* pd_filter, int size)
{
	for (int i = size - 1; i > 0 ; i--)
	{
		pd_filter[i] = pd[i] - pd[i - 1];
	}
	pd_filter[0] = pd_filter[1];
}
